...
for i in range(0, 500):
    ...
    if int(i) == 0:
	api = str(url) + '?regions=National&date=21-10-16&gender=' + gen + '&sub_category=' + cat_prof_a + '&duration=' + str(dur) + '&spot_duration=' + str(spot_dur) + '&budgets=' + str(bud) + '&user_id=radhika@amagi.com'
    else:
...
#	    print r[0]
	...
	for document in em:
	    ...
	    return result
    def special_val_reach_spli_mongo(region_name,coverage):
...
#		    print audience_type
		    ...
		    print 'tvt_from_mongo',tvt_from_mongo,region_name,channel_name,age,g,nccs
...
#			print cpv,cpv_r
			...
			impr_2 = 0
			impr_2_n = 0
			if 'detailed_region_impressions' not in  data['package']['region'][i]:
			    tvt_from_mongo = tvt_mongo_new(region_name,channel_name,age,g,nccs)
                            tvt =  float(tvt_from_mongo) * 1000 * int(rotates_out)
			    print 'tvt',tvt,tvt_from_mongo
			    if type_out == 'Spliced':
				region_name = 'DTH'	
...
#			    print coverage
			    if str(coverage) == 'Cable':
                            	value = special_val_reach_spli_mongo('National','Cable')
                            elif str(coverage) == 'DTH':
                            	value = special_val_reach_spli_mongo('National','DTH')
                            elif str(coverage) == 'Cable+DTH':
                                value = 100
			    print tvt, value
			    impr = tvt * (value / 100)
