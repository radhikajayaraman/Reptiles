...
print "Result_Discount,Result_bud_overshoot,Result_actual_cost,Result_Cost,Discount_Percentage,Budget_shoot_status,Actual_cost_computed,Actual_cost_response,Type,total_cost,package_cost,Budget_input,Budget_Difference,Discount,input_command"
...
for cnt in range(0, 500):
    ...
    command_str = " python " + path + "compute_media_packages_v3.py -f $HOME -d 21-10-16 -a 15+"  " -g " + gender_str +  \
              " -r "  + current_regions_str +   cat_prof +" -p " + str(random.randint(5,20)) + " -s " + str(spot_duration[random.randint(0,2)]) + " -b "  + str(random.randint(0,3000000))
#    print command_str
        ...
        #Comparing actual cost from response and computation
                    ...
                    if type_r == "Spliced":
                        ...
                        if actual_cost_r >= (0.96 * actual_cost_computed) and actual_cost_r <= (1.25 * actual_cost_computed):
                            ...
                            res2 = "Fail"
    print res,',',res1,',', res2 , ',',res3 ,',',d,',',status+str(p),',',a+str(actual_cost_computed),',',aa+str(actual_cost_r),',',type_r,',',total_cost,',',package_cost,',',budget_input,',',budget_diff,',',discount,',',input_command
