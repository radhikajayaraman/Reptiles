...
#	print regions
	...
	if 'Mumbai' in regions: regions.remove('Mumbai')
	if 'Rest of Maharashtra' in regions: regions.remove('Rest of Maharashtra')
	if 'National' in regions: regions.remove('National')
	if 'DTH' in regions: regions.remove('DTH')
#	print regions
#	regions = ["Pun/Har/Cha/HP/J%26K","Mumbai","Rest of Maharashtra","West Bengal","DTH","Maharashtra/Goa","Hyderabad","Bangalore","North East","Orissa","Rajasthan","MP/Chhattisgarh","Gujarat","Bihar/Jharkhand","Uttar Pradesh","Delhi NCR","Kerala","TN/Pondicherry","Karnataka","National"]
...
#-            category_m = amagi_t[0]
	    ...
	    continue
	category_used = (z['adex_super_category'])
	sub_category_used = (z['adex_sub_category'])
        if "&" in category_used:
...
#        print category_m, sub_category_m,prof_m
        api = str(url)+'?regions='+mul_random_regs()+'&date=21-10-16&gender='+gen+'&category='+category_used+'&sub_category='+sub_category_used+'&profile='+prof_m+'&duration='+str(dur)+'&spot_duration='+str(spot_dur)+'&budgets='+str(bud)+'&user_id=radhika@amagi.com'
...
#    	    print budget_out
#### Response package_cost
	    package_out = data['package']['package_cost']
#### Response tax
...
####Basic check checks the gender,spot_duration, 15% tax amount
	    ...
	    print "gender_in", str(gender_in) , "gender_out",str(gender_out), "profile_in", str(profile_in) , "spot_duration_in", str(spot_duration_in),"spot_duration_out", str(spot_duration_out),"tax_out",tax_out,"percent",percent
...
#	    print abs(bud_diff)
		...
		if bud_diff_percent > 5:
		    print 'Overshoot',',','bud_diff_percent : '+str(bud_diff_percent),',',budget_in,',',budget_out,',','package_cost',',',package_out,',',
		else:
		    print 'No_Overshoot',',','bud_diff_percent : '+str(bud_diff_percent),',',budget_in,',',budget_out,',','package_cost',',',package_out,',',
	    elif bud_diff < 0:
		if bud_diff_percent > 5:
		    print 'Under_Utilisation',',','bud_diff_percent : '+str(bud_diff_percent),',',budget_in,',',budget_out,',','package_cost',',',package_out,',',
                else:
                    print 'No_Under_Utilisation',',','bud_diff_percent : '+str(bud_diff_percent),',',budget_in,',',budget_out,',','package_cost',',',package_out,',',


	def budget_shoot_above10():
            bud_diff = int(budget_out) - int(budget_in)
#           print abs(bud_diff)
            #### Checking bud_diff is wat percent of input budget
            bud_diff_percent = round(((abs(bud_diff)*100)/int(budget_in)),2)
            if bud_diff >= 0:
                if bud_diff_percent > 10:
                    print 'Overshoot',',','bud_diff_percent : '+str(bud_diff_percent),',',budget_in,',',budget_out,',','package_cost',',',package_out,',',api
                else:
		    print "Nothing"
#                   print 'No_Overshoot',',','bud_diff_percent : '+str(bud_diff_percent),',',budget_in,',',budget_out,',','package_cost',',',package_out,',',
            elif bud_diff < 0:
                if bud_diff_percent > 10:
                    print 'Under_Utilisation',',','bud_diff_percent : '+str(bud_diff_percent),',',budget_in,',',budget_out,',','package_cost',',',package_out,',',api
                else:
		    print "Nothing"
#                    print 'No_Under_Utilisation',',','bud_diff_percent : '+str(bud_diff_percent),',',budget_in,',',budget_out,',','package_cost',',',package_out,',',
...
#                                   print 'impr_2_nr',impr_2_nr
	...
	def cutoff_rate_check():
            ...
            if final == min_pakg_cost or min_pakg_cost == (final - 1) or min_pakg_cost == (final + 1):
                ...
                print 'Cutoff-Fail',',',
#        basic = basic_check()
	secs = float(end_time) - float(start_time)
	print 'time', start_time, end_time, secs
#	budge = budget_shoot()
#	budg = budget_shoot_above10()
