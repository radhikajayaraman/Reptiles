...
import re
import pdb
global data, sum_c, total_impr, sum_impr,g, impr,summa
#print "Tax,Gender,Budget_Shoot,Budget_diff_percentage,Result_Cost,Result_Discount,Result_Cutoff,Input_command"
print "Result_impression, Impression_from_response,Impression_summation,Input_command"
##config file
...
#            print profile_in
	...
	for i in range(0,len(data['package']['region'])):
	    reg_impr = data['package']['region'][i]['region_impressions']
	    summa += reg_impr 
	pakg_impr = data['package']['package_impressions']
	summ = round(summa)
	pkg = round(pakg_impr)
	if summ == pkg or summ == (pkg - 1) or summ == (pkg + 1):
	    print "Pass",',',pkg,',',summ,',',input_command
	else:
	    print "Fail",',',pkg,',',summ,',',input_command
    pkg_imp = san_summa_impr()
